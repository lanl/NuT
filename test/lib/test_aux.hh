...
void
post_report(std::string const & target,
            std::string const & aspect,
            bool passed);
...
bool
check_one_changed(tally_t const & tally,
                  tally_t const & ref,
...
bool
check_two_changed(tally_t const & tally,
                  tally_t const & ref,
...
template <typename v_t>
bool
check_same_v(v_t const * const pv,
             v_t const * const pr,
...
bool
check_same_unless_changed(v_t1 const * const v,
                          v_t1 const * const r,
                          v_t2 const * const c)
{
  return ((void *)v != (void *)c) ? check_same(v, r) : true;
}
...
bool
check_same_unless_2changed(v_t1 const * const v,
                           v_t1 const * const r,
                           v_t2 const * const c1,
                           v_t3 const * const c2)
{
...
bool
check_same_verb(v_t const * const pv, v_t const * const pr, comp c)
{
  v_t const & v(*pv);
  v_t const & r(*pr);
  bool same(true);
...
struct comp_verb {
  bool operator()(fp_t const & val, fp_t const & ref)
  {
    ...
    if(!passed) {
      std::cout << this->name_ << std::setprecision(15) << ": val = " << val
                << ", ref = " << ref << std::endl;
    }
    return passed;
  }
...
bool
tallies_same(tally_t const & t1, tally_t const & t2)
{
  bool same(true);
                  ...
                  v_t const * const changed_v)
{
  ...
  same =
      same && check_same_unless_changed(&tally.energy, &ref.energy, changed_v);
  same = same &&
         check_same_unless_changed(&tally.momentum, &ref.momentum, changed_v);
  same = same && check_same_unless_changed(&tally.ew_escaped, &ref.ew_escaped,
                                           changed_v);
  same = same && check_same_unless_changed(&tally.n_n, &ref.n_n, changed_v);
  same = same && check_same_unless_changed(&tally.n_p, &ref.n_p, changed_v);
  same = same &&
         check_same_unless_changed(&tally.n_e_minus, &ref.n_e_minus, changed_v);
  same = same &&
         check_same_unless_changed(&tally.n_e_plus, &ref.n_e_plus, changed_v);
  same = same && check_same_unless_changed(&tally.ew_n, &ref.ew_n, changed_v);
  same = same && check_same_unless_changed(&tally.ew_p, &ref.ew_p, changed_v);
  same = same && check_same_unless_changed(&tally.ew_e_minus, &ref.ew_e_minus,
                                           changed_v);
  same = same &&
         check_same_unless_changed(&tally.ew_e_plus, &ref.ew_e_plus, changed_v);
  same = same &&
         check_same_unless_changed(&tally.n_escape, &ref.n_escape, changed_v);
  same = same &&
         check_same_unless_changed(&tally.n_reflect, &ref.n_reflect, changed_v);
  same = same && check_same_unless_changed(&tally.n_cell_bdy, &ref.n_cell_bdy,
                                           changed_v);
  same = same &&
         check_same_unless_changed(&tally.n_cutoff, &ref.n_cutoff, changed_v);
                  ...
                  v_t2 const * const chgd_v2)
{
  ...
  // for each tally field, if not the one in question,
  // make sure that the tally and ref fields are the same
  same = same && check_same_unless_2changed(&tally.energy, &ref.energy, chgd_v1,
                                            chgd_v2);
  same = same && check_same_unless_2changed(&tally.momentum, &ref.momentum,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.ew_escaped, &ref.ew_escaped,
                                            chgd_v1, chgd_v2);
  same = same &&
         check_same_unless_2changed(&tally.n_n, &ref.n_n, chgd_v1, chgd_v2);
  same = same &&
         check_same_unless_2changed(&tally.n_p, &ref.n_p, chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_e_minus, &ref.n_e_minus,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_e_plus, &ref.n_e_plus,
                                            chgd_v1, chgd_v2);
  same = same &&
         check_same_unless_2changed(&tally.ew_n, &ref.ew_n, chgd_v1, chgd_v2);
  same = same &&
         check_same_unless_2changed(&tally.ew_p, &ref.ew_p, chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.ew_e_minus, &ref.ew_e_minus,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.ew_e_plus, &ref.ew_e_plus,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_escape, &ref.n_escape,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_reflect, &ref.n_reflect,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_cell_bdy, &ref.n_cell_bdy,
                                            chgd_v1, chgd_v2);
  same = same && check_same_unless_2changed(&tally.n_cutoff, &ref.n_cutoff,
                                            chgd_v1, chgd_v2);
  same =
  ...
  same = same &&
         check_same_unless_2changed(&tally.n_nu_e_bar_pos_scat,
  ...
  same = same &&
         check_same_unless_2changed(&tally.n_census_nu_x_bar,
  ...
  same = same &&
         check_same_unless_2changed(&tally.ew_census_nu_e_bar,
  ...
  same = same &&
         check_same_unless_2changed(&tally.ew_census_nu_x_bar,
