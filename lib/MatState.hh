...
namespace nut
{
    namespace P // transformations specific to the p.1-p.7 file format
    {
        ...
        template <typename fp_t>
        state_entry_t<fp_t> row_to_state_entry(MatStateRowP<fp_t> const & row);
    ...
    struct MatState
    {
        ...
        typedef Velocity<fp_t>     Velocity_T;
        ...
        explicit MatState(std::vector< MatStateRowP<fp_t> > const & rows);

    private:
        void add_entry( P::state_entry_t<fp_t> const & entry, size_t const pos);
    }; // MatState

    // ctor

    template <typename fp_t>
    MatState<fp_t>::MatState(std::vector< MatStateRowP<fp_t> > const & rows)
        ...
        , velocity(rows.size())
    {
        ...
        for(size_t i = 0; i < rows.size(); ++i)
        {
            ...
            P::state_entry_t<fp_t> entry = P::row_to_state_entry(rows[i]);
    ...
    template <typename fp_t>
    void 
    MatState<fp_t>::add_entry(P::state_entry_t<fp_t> const & entry, 
                              size_t const i
        )
    {
    ...
    {
        template <typename fp_t>
        fp_t extract_radius(MatStateRowP<fp_t> const & row){return row.radius;}
    }
    ...
    * convention. */
    template <typename mesh_t, typename fp_t>
    mesh_t rows_to_mesh(std::vector<MatStateRowP<fp_t> > const & rows,
                        fp_t const llimit,
                        fp_t const ulimit,
                        size_t & llimitIdx,
                        size_t & ulimitIdx
        )
    {
        ...
        std::transform(rows.begin(),rows.end(),rads.begin(),extract_radius<fp_t>);
    ...
    namespace P
    {
         ...
         * from a MatStateRow */
        template <typename fp_t>
        dens_t<fp_t> row_to_rho(MatStateRowP<fp_t> const & row)
        {
            dens_t<fp_t> r;
        ...
        template <typename fp_t>
        temp_t<fp_t> row_to_temp(MatStateRowP<fp_t> const & row)
        {
            temp_t<fp_t> t;
        ...
        template <typename fp_t>
        lum_t<fp_t> row_to_lum(MatStateRowP<fp_t> const & row)
        {
            lum_t<fp_t> l;
        ...
        template <typename fp_t>
        vel_t<fp_t> row_to_vel(MatStateRowP<fp_t> const & row)
        {
            vel_t<fp_t> v;
            v.v = row.velocity;
            return v;
