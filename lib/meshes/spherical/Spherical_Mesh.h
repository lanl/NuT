...
public:
  ...
  static Intersection_T dist_to_bdy_impl(geom_t x, geom_t omega, geom_t rlo,
                                         geom_t rhi);

  Ray advance_point(Point const &init_pnt, Vector const &dir,
  ...
  explicit Spherical_1D_Mesh(std::vector<geom_t> &&bdys)
      : m_cell_bounds(std::move(bdys)),
        m_num_cells(
            (/*Require(m_cell_bounds.size() > 1,""),*/ m_cell_bounds.size() -
             1)) {}
