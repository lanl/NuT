...
Cartesian_Mesh::reflect(face_handle_t const & f, Vector const & v) const
{
  Vector reflected{v};
...
Cartesian_Mesh::face_to_face_name(Cell const & c, face_handle_t const & f) const
{
  // Need to know direction, and whether it is high or low
  Plane p{face_to_plane(f.as_id())};
  // return the corresponding normal
  Cell possible_cell{face_to_cell(f)};
  bool is_high_face(possible_cell.as_id() != c.as_id());
  Face_Name fname{LOW_X};
  if(is_high_face) {
    switch(p) {
      case YZ: fname = HIGH_X; break;
      case XZ: fname = HIGH_Y; break;
      case XY: fname = HIGH_Z; break;
    ...
    switch(p) {
      case YZ: fname = LOW_X; break;
      case XZ: fname = LOW_Y; break;
      case XY: fname = LOW_Z; break;
      case NUM_PLANES:  // some hideous error here
        break;
    }
  }
...
inline Cartesian_Mesh::Vector
Cartesian_Mesh::get_normal(Cell const & c, face_handle_t const & f) const
{
...
inline Cartesian_Mesh::Vector
Cartesian_Mesh::get_normal(Face_Name f)
{
...
inline Cartesian_Mesh::Vector
Cartesian_Mesh::sample_position(RNG_T & rng, Cell const & c) const
{
...
inline Cartesian_Mesh::Cell
Cartesian_Mesh::cell_across(Cell const & c,
                            face_handle_t const & f,
                            Point const & /*p*/) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
  switch(f.get_index()) {
    case LOW_X: ix = (ix > 0) ? ix - 1 : void_cell_idx; break;
    case HIGH_X: ix = (ix < nx_ - 1) ? ix + 1 : void_cell_idx; break;
    case LOW_Y: iy = (iy > 0) ? iy - 1 : void_cell_idx; break;
    case HIGH_Y: iy = (iy < ny_ - 1) ? iy + 1 : void_cell_idx; break;
    case LOW_Z: iz = (iz > 0) ? iz - 1 : void_cell_idx; break;
    case HIGH_Z:
...
Cartesian_Mesh::volume(Cell const & /*c*/) const
{
  return dx_ * dy_ * dz_;
}
...
Cartesian_Mesh::find_cell(Point const & p) const
{
  ...
  Cell const c(cartesian_to_linear(ix, iy, iz, nx_, ny_));
  // Require(in_cell(p, c));
  return c;
}  // find_cell

inline bool
Cartesian_Mesh::valid_cell(Cell const & c) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
...
Cartesian_Mesh::get_x_extents(Cell const & c) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
...
inline Cartesian_Mesh::Extents_T
Cartesian_Mesh::get_y_extents(Cell const & c) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
...
inline Cartesian_Mesh::Extents_T
Cartesian_Mesh::get_z_extents(Cell const & c) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
...
inline geom_t
Cartesian_Mesh::compute_distance(geom_t d_cos,
                                 geom_t coord,
...
Cartesian_Mesh::intersection(Ray const & r, Cell const & c) const
{
  using nut::Max_Dbl;
  ...
  if(dx <= dy) { dist = dx, f = dir[0] < 0.0 ? LOW_X : HIGH_X; }
  else {
    dist = dy, f = dir[1] < 0.0 ? LOW_Y : HIGH_Y;
  }
  if(dz < dist) { dist = dz, f = dir[2] < 0.0 ? LOW_Z : HIGH_Z; }
  // Check(dist < Max_Dbl);
  // Check(f < 6);
...
inline bool
Cartesian_Mesh::in_cell(Vector const & p, Cell const & c) const
{
...
inline index_t
Cartesian_Mesh::num_cells() const
{
  return nx_ * ny_ * nz_;
}
...
{
  return Cell{cartesian_to_linear(ix, iy, iz, nx_, ny_)};
}

// private methods

/**\brief Compute a Cartesian_Face index from a Cell index and one of the six
 * Face Names.
 * Note: must be called with a mesh-valid cell; cannot be called with +1 in
 * any domension, for instance.
 */
inline Cartesian_Face
Cartesian_Mesh::cell_to_face(Cell const & c, Face_Name const fn) const
{
  auto [ix, iy, iz] = linear_to_cartesian(c.as_id(), nx_, ny_);
 ...
 */
inline Cartesian_Face
Cartesian_Mesh::cell_to_face(index_t ix,
                             index_t iy,
                             index_t iz,
                             Plane const p) const
{
  ...
  for(uint32_t i = 0; i < ip; ++i) { base += n_faces[i]; }
...
inline Plane
Cartesian_Mesh::face_to_plane(index_t const face_idx) const
{
...
Cartesian_Mesh::face_to_plane_offset(index_t const face_idx) const
{
  ...
  // still makes sense.
  index_t offset{0ull};
  if(face_idx >= n_faces[0]) { offset += n_faces[0]; }
  if(face_idx >= n_faces[1] + n_faces[0]) { offset += n_faces[1]; }
  return offset;
}
...
inline bool
Cartesian_Mesh::is_boundary(face_handle_t f) const
{
...
inline Vertex
Cartesian_Mesh::cell_to_vertex(index_t ix, index_t iy, index_t iz) const
{
...
inline void
Cartesian_Mesh::make_cartesian_mesh()
{
...
struct Cartesian_Mesh::boundary_face_iterator
  ...
  {
    Cell c{m.make_cell(ix, iy, iz)};
    face_handle_t f{m.cell_to_face(c, f_n)};
    return f;
  }

  boundary_face_iterator(index_t ix_,
                         index_t iy_,
                         index_t iz_,
                         index_t xmin_,
                         index_t xmax_,
