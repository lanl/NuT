...
public:
  // void set_boundary(Face_Name const & f, boundary_types::BDY_TYPE const & b);
  ...
  bool is_boundary(Face_Name const & f, boundary_types::BDY_TYPE const & b) const;

  /**\brief Is the face f set to boundary b? */
  boundary_types::BDY_TYPE get_boundary(Face_Name const & f) const;
...
inline void
Cell_Face_Descriptor<Face_Name, mxfcs>::set_boundary(
    Face_Name const & f,
    boundary_types::BDY_TYPE const & b) {
  if (is_boundary(f, b)) {
    clear_boundary(f);
  }
  // 3 bits per face
...
template<typename Face_Name, size_t mxfcs>
inline void
Cell_Face_Descriptor<Face_Name, mxfcs>::clear_boundary(
    Face_Name const & f) {
...
template<typename Face_Name, size_t mxfcs>
inline bool
Cell_Face_Descriptor<Face_Name, mxfcs>::is_boundary() const {
  return d_ != boundary_types::NONE;
}

template<typename Face_Name, size_t mxfcs>
inline bool
Cell_Face_Descriptor<Face_Name, mxfcs>::is_boundary(
    Face_Name const & f) const {
  boundary_types::BDY_TYPE fbase{get_boundary(f)};
  return fbase != boundary_types::NONE;
}
...
Cell_Face_Descriptor<Face_Name, mxfcs>::is_boundary(
    Face_Name const & f,
    boundary_types::BDY_TYPE const & b) const {
  boundary_types::BDY_TYPE fbase{get_boundary(f)};
  return fbase == b;
}

template<typename Face_Name, size_t mxfcs>
inline boundary_types::BDY_TYPE
Cell_Face_Descriptor<Face_Name, mxfcs>::get_boundary(
    Face_Name const & f) const {
  index_t const shifter = 3 * f;
  index_t fbase = ((static_cast<index_t>(d_)) >> shifter) & 7;
  return boundary_types::BDY_TYPE{fbase};
}
