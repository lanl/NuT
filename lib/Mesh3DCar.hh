...
namespace nut
{
    ...
    struct ijk_t
    {
        ...
        cell_t j;
        cell_t k;
        ijk_t(cell_t i_,cell_t const j_, cell_t const k_)
            : i(i_),j(j_),k(k_){}
    };
     ...
     */
    template <typename cell_t, typename geometry_t,
    ...
    public:
        ...
        inline
        cell_t
        getBCIndex(cell_t const i1, cell_t const i2, cell_t const n1)
        {
            return i1 + n1 * i2;
        }
         ...
         */
        bdy_desc_t
        getBC(face_t const face, cell_t const cidx) const
        {
        ...
        geom_t
        volume(cell_t const /* ignored! */) const {
        ...
        inline
        cell_t
        getIJKComp(ijk_T const & ijk, face_t const face)
        {
        ...
        inline
        cell_t
        getIJKComp(ijk_T const & ijk, dir_t const d)
        {
            cell_t result(0);
            switch(d)
            {
        ...
        inline
        cell_t getDimSize(face_t const face) const
        {
        ...
        inline
        bool isBoundary(cell_t const cidx, face_t const face) const
        {
            ijk_T const ijk(mkIJK(cidx));
        ...
        inline
        bool isBoundary(ijk_T const & ijk, face_t const face) const
        {
            bool isBound(false);
        ...
        static
        inline
        ijk_T updateIJK(ijk_T const & old, cell_t const newIdx, face_t const f)
        {
         ...
         */
        cell_t
        cell_across_face(cell_t const cell,face_t face) const {
            // convert to i,j,k
        ...
        inline
        geom_t
        getDeltaCoord(dir_t const d) const
        {
            geom_t res(0);
            switch(d)
            {
         ...
         * 0 < cell <= n_cells                              */
        extents_t
        cell_extents(cell_t const cell,dir_t const d) const
        {
        ...
        coord_t
        new_coordinate(coord_t const oldc, geom_t const distance) const
        {
        ...
        d_to_b_t
        distance_to_bdy(coord_t const crd,
                        ...
                        cell_t const cell) const
        {
            ...
            d_to_b_t const dz = omega.v[2] > 0 ?
                ...
                omega.v[2] < 0 ?
                d_to_b_t((zs.first - x.v[2]) / omega.v[2],low_z) :
