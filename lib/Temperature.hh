...
struct Temperature {
   ...
   */
  Temperature(vf const & T_p_, vf const & T_e_minus_, vf const & T_e_plus_)
      : T_p(T_p_), T_e_minus(T_e_minus_), T_e_plus(T_e_plus_)
  {
    this->check_sizes();
    return;
  }
  ...
  explicit Temperature(Temperature const & t)
      : T_p(t.T_p), T_e_minus(t.T_e_minus), T_e_plus(t.T_e_plus)
  {
  }

  /*!\brief construct empty Temperature of given size */
  explicit Temperature(size_t const n)
      : T_p(n, fp_t(0)), T_e_minus(n, fp_t(0)), T_e_plus(n, fp_t(0))
  {
  }
