...
namespace nut
{
    ...
    public:
        ...
        {
            key_t key;
            key.v[0] = lo;  key.v[1] = hi;
            return key;
        }

        static
        ctr_t make_ctr(csd_t w4,csd_t w3,csd_t w2,csd_t w1)
        {
            ctr_t c;
            c.v[0] = w4;
            c.v[1] = w3;
    ...
    public:
        ...
        bool operator==(Buffer_RNG const & r) const {
            bool same = 
                ...
                m_n_warns == r.m_n_warns &&
                m_max_warns == r.m_max_warns &&
                m_warn_next == r.m_warn_next;
            return same;
        }
    ...
    fp_t Buffer_RNG<fp_t>::random(){
        ...
        {
            std::cerr << "Buffer_RNG rolled over from " << m_old_idx
                      << " to " << m_idx << std::endl;
            m_warn_next = false;
            m_n_warns += 1;
        }
...
#define NTAB 32
    ...
    private:
        ...
        float ranf_core(){
            ...
            if(j >= NTAB)
            {
                std::cerr << "index j out of range! j = " << j
                          << ", NTAB = " << NTAB << std::endl;
            }
